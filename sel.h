Matrix createLocalK(int element, mesh &m)
{
    ...
    float e = m.getParameter(E_CONSTANT),
          a = m.getParameter(A_CONSTANT),
...
{
    Vector b;
    float f = m.getParameter(F_CONSTANT),
          l = m.getNode(element + 1).getX() - m.getNode(element).getX();
...
{
    float e = m.getParameter(E_CONSTANT), a = m.getParameter(A_CONSTANT);
